...
class SwapTx(Tx):
    def __init__(self, tx: Tx):
        ...
        if 'path' in self.decoded_input['func_params']:
            ...
            self.token_from_qty = self.decoded_input['func_params'].amountIn if hasattr(self.decoded_input['func_params'],
                                                                                        'amountIn') else self.value

            # output qty's key may vary
            self.token_to_qty = self.decoded_input['func_params'].amountOutMin if hasattr(
                self.decoded_input['func_params'], 'amountOutMin') else None
            self.token_to_qty = self.decoded_input['func_params'].amountOut if hasattr(
                self.decoded_input['func_params'], 'amountOut') else self.token_to_qty
