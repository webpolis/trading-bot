...
class MetricsCalculator:
    ...
    def get_whale_txs(self, whale, df_txs):
    ...
    def get_txs_until_last_date(self, scrappers_instance, last_date):
        """
        ...
        page_number = 1
        txs_df = scrappers_instance.get_txs(page_number)
        frames = [txs_df]
        row_number = 0
        date_row_number = txs_df['\n\nAge\n\n'][row_number]
        date_row_number = datetime.strptime(date_row_number, "%Y-%m-%d %H:%M:%S").\
            replace(tzinfo=pytz.UTC).astimezone(pytz.timezone("America/Buenos_Aires")).replace(tzinfo=None)
        ...
        while (date_row_number - last_date).total_seconds() > 0:
            print(date_row_number, last_date)
            ...
            while(row_number < len(txs_df)):
                ...
                date_row_number = datetime.strptime(txs_df['\n\nAge\n\n'][row_number], "%Y-%m-%d %H:%M:%S").\
            replace(tzinfo=pytz.UTC).astimezone(pytz.timezone("America/Buenos_Aires")).replace(tzinfo=None)
                row_number += 1
            page_number += 1
