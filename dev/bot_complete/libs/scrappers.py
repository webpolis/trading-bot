...
class Scrappers:
    ...
    def get_txs(self, token, page_number):
        url_to_txs_page_number = self.tokens[token.name].url_txs + str(page_number)
        ...
        if token.network == 'ethereum':
            ...
            table_txs_html = soup_data.findAll("table", {"class": "table table-striped"})[0]
            txs_df = self.cardano_getter.txs_table_to_df(table_txs_html)
        return txs_df
    ...
    def get_whales(self, token, number_of_whales):
        ...
        while len(addresses_df) < number_of_whales:
            url_to_top_addresses_page_number = self.tokens[token.name].url_whales + str(page_number)
            time.sleep(3)
